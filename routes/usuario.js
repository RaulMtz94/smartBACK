...
//REGISTROS DE USUARIOS APLICACION MOVIL

app.post('/movil', (req, res) => {
    var body = req.body;
    var newCustomer = {
        "name":body.nombre,
        "email":body.email,
    ...
    openpay.customers.create(newCustomer, function(error, cuerpo) {
        error;    // null if no error occurred (status code != 200||201||204)
        cuerpo;  
        console.log("respuesta",cuerpo.id);

   
          // contains the object returned if no error occurred (status code == 200||201||204)
