...
//NUEVOOO
// ==========================================
// Obtener todos los sorteos TERMINADOS
// ==========================================
app.get('/terminados', (req, res, next) => {
    var datestart = req.query.datestart;
    var dateend = req.query.dateend;

    Sorteo.find({estatus: "0", $and: [ { fechavence: { $gte: new Date(datestart) } }, { fechavence: { $lte: new Date(dateend) } } ]} )
     ...
     .exec(
         (err, sorteos) => {
             ...
             Sorteo.count({estatus: "0", $and: [ { fechavence: { $gte: new Date(datestart) } }, { fechavence: { $lte: new Date(dateend) } } ]}, (err, conteo) => {
                 res.status(200).json({
                     ok: true,
                     sorteos: sorteos,
                     total: conteo
